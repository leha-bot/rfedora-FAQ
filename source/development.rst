...
Запросим новый side-tag для F36:
...
Внимание! Каталоги с исходниками всех участников группы должны находиться на одном уровне файловой иерархии. При сборке для side-tag, перед запуском процесса необходимо зайти в каталог каждого пакета из цепочки и при помощи команды ``fedpkg switch-branch f36`` переключиться на нужную ветку.
...
.. code-block:: text
    ...
    cd bar
    fedpkg switch-branch f36
    fedpkg chain-build --target f36-build-side-XXXXX libfoo1 libfoo2
...
.. code-block:: text
    ...
    cd bar
    fedpkg switch-branch f36
    fedpkg chain-build --target f36-build-side-XXXXX libfoo1 : libfoo2 libfoo3 :
...
Если вам необходимо переложить несколько коммитов из одной ветки в другую (например, вы сделали некоторые правки в вашей ветке, которые не относятся к разрабатываемой вами функциональности), то мы можем запросить список из N коммитов из первой ветки, и сделать **cherry-pick** на второй ветке. Эти действия можно схлопнуть в одну конвейерную команду (пример приведен для 10 коммитов из ветки mybranch, его надо выполнять в другом каталоге, созданном через **git worktree add**. Пример приведен для Bash-совместимых shell, в которых также есть команды **tac** и **xargs**):


.. code-block:: text

    git log --format=%h -10 mybranch | tac | xargs git cherry-pick

Указанная команда сначала запрашивает список из коммитов в формате "список хешей", затем разворачивает список с помощью команды **tac**, и затем каждую строчку списка передает команде git cherry-pick, которая впоследствие и применяет каждый коммит в нужную ветку.
